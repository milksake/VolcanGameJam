...
var lift1_mesh : MeshInstance3D:
	get():
		if lift1_mesh:
			return lift1_mesh
		
		var result = get_node_or_null("Lift1")
...
var lift2_mesh : MeshInstance3D:
	get():
		if lift2_mesh:
			return lift2_mesh
		
		var result = get_node_or_null("Lift2")
...
var ring_mesh : MeshInstance3D:
	get():
		...
		var result = get_node_or_null("Ring")
...
var mist_mesh : MeshInstance3D:
	get():
		if mist_mesh:
			return mist_mesh
		
		var result = get_node_or_null("Mist")
...
@export var area_radius : float = 1.0:
	set(v):
		area_radius = v
		if lift1_mesh:
			lift1_mesh.mesh.bottom_radius = area_radius
			lift1_mesh.mesh.top_radius = area_radius + 0.2
		if lift2_mesh:
			lift2_mesh.mesh.bottom_radius = area_radius
			lift2_mesh.mesh.top_radius = area_radius + 1.0
		if ring_mesh:
			ring_mesh.mesh.size = Vector2(4.0 * area_radius, 4.0 * area_radius)
		if mist_mesh:
			mist_mesh.mesh.size = Vector2(4.0 * area_radius, 4.0 * area_radius)
